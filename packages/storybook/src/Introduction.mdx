...
- **@dsn-starter-kit/design-tokens**: Design tokens voor kleuren, typografie, spacing en meer
- **@dsn-starter-kit/core**: Core utilities, reset styles en helper functies
- **@dsn-starter-kit/components-html**: Pure HTML/CSS componenten
- **@dsn-starter-kit/components-react**: React wrapper componenten
- **@dsn-starter-kit/components-web**: Web Components (native custom elements)
...
- **BreakoutSection**: Laat een sectie uitslaan buiten een beperkte paginabreedte om de volledige viewportbreedte te beslaan
...
- **Inputs**: TextInput, FileInput, TextArea, EmailInput, PasswordInput, NumberInput, TelephoneInput, SearchInput, TimeInput, DateInput, Select
...
- **BasePage**: Volledige paginastructuur met `Body`, `SkipLink`, `PageLayout`, `PageHeader`, `PageBody` en `PageFooter`: fundament voor alle verdere paginatemplates; bevat ook Grid- en BreakoutSection-varianten
- **FormStepPage**: Paginatemplate voor meerstappenformulieren; header toont alleen logo, formulierinhoud gecentreerd via `colStartLg`/`colEndLg` op kolommen 3–10
...
- **WithSidebarPage**: Paginatemplate met een vaste zijkolom voor subnavigatie (Level 2 MenuLinks), zichtbaar vanaf het large breakpoint (64em)
